    ...
    if os.path.exists("vault_embeddings.pkl"):
        ...
        for content in vault_content:
            response = ollama.embeddings(
                model='mxbai-embed-large', prompt=content)
...
def get_relevant_context(rewritten_input, vault_embeddings, vault_content, top_k=3):
    ...
    input_embedding = ollama.embeddings(
        model='mxbai-embed-large', prompt=rewritten_input)["embedding"]
    print(input_embedding)
    # Compute cosine similarity between the input and vault embeddings
    cos_scores = torch.cosine_similarity(torch.tensor(
...
def chat(user_input, vault_embeddings, vault_content):
    ...
    relevant_context = get_relevant_context(
        user_input, vault_embeddings, vault_content)
